...
namespace Com.StellmanGreene.FindRelated
{
    public class RelatedFinder
    {
        ...
        public void Go(string odbcDsn, string relatedTable)
        {
            ...
            DataTable table = db.ExecuteQuery("SELECT setnb, pmid FROM peoplepublications");
            ...
            foreach (string setnb in peopleIds.Keys)
            {
                ...
                Dictionary<int, List<int>> relatedSearchResults = GetIdsFromXml(xml);
                ...
                foreach (int authorPublicationPmid in relatedSearchResults.Keys)
                {
                    ...
                    foreach (Publication relatedPublication in publications.PublicationList)
                    {
                        ...
                        Console.WriteLine("{0} - {1}", authorPublicationPmid, relatedPublication.PMID);

                        // Only write the related publication to the database if it hasn't already been added to the related publications table.
                        ...
                        int relatedCount = db.GetIntValue("SELECT Count(*) FROM " + relatedTable + " WHERE PMID = (?)",
                            new System.Collections.ArrayList() { Database.Parameter(relatedPublication.PMID) });
                        if (relatedCount == 0)
                        {
                            ...
                            if (relatedPublication.Authors != null)
                            {
                                try
                                {
                                    ...
                                    db.ExecuteNonQuery(
                                        "INSERT INTO " + relatedTable + " (PMID, RelatedPMID) VALUES (?, ?)",
                                        new System.Collections.ArrayList() { Database.Parameter(authorPublicationPmid), Database.Parameter(relatedPublication.PMID) });
                                }
                                catch (Exception ex)
                                {
        ...
        private static Dictionary<int, List<int>> GetIdsFromXml(string xml)
        {
            ...
            foreach (XmlNode linkSet in linkSets)
            {
                ...
                if (int.TryParse(linkSet["IdList"]["Id"].InnerText, out pmid)) {
                    ...
                    foreach (XmlNode linkSetDb in linkSetDbs)
                    {
                        ...
                        {
                            // We've found the link set of related PubMed publications. Add it to ther esults.
                            List<int> linkList;
