...
namespace Com.StellmanGreene.PubMed.Unit_Tests
{
    ...
    public class TestReports
    {
        ...
        public void TestEntireReport()
        {
            ...
            // Read the people report back in 
            string ConnectionString =
                "Driver={Microsoft Text Driver (*.txt; *.csv)};Dbq="
                + AppDomain.CurrentDomain.BaseDirectory + ";";
            OdbcConnection Connection = new OdbcConnection(ConnectionString);
            OdbcDataAdapter DataAdapter = new OdbcDataAdapter
                ("SELECT * FROM [TestEntireReport.csv]", Connection);
            DataTable Report = new DataTable();
            int Rows = DataAdapter.Fill(Report);
            Connection.Close();

            // Make the file contains the correct number of rows
            Assert.IsTrue(Rows == 85);

            // Set the primary keys for the report so CheckData can use Find() to search for rows
            Report.PrimaryKey = new DataColumn[] { Report.Columns["Setnb"], Report.Columns["Year"] };
            ...
            for (int RowNum = 0; RowNum < HandGeneratedData.Rows.Count; RowNum++)
            {
                ...
                DataRow ReportRow = Report.Rows.Find(new object[] { Setnb, Year });

                for (int i = 2; i < Columns.Length; i++)
                {
                    String Column = Columns[i];
                    if (Column.StartsWith("wghtd_"))
                    {
                        float Value = Convert.ToSingle(HandGeneratedRow[Column]);
                        float Compare = Convert.ToSingle(ReportRow[Column]);
                        Assert.AreEqual(Compare, Value, Setnb + "/" + Year.ToString() + "/" + Column + " -- hand generated has " + Value.ToString() + ", report has" + Compare.ToString());
                    }
                    else
                    {
                        int Compare = Convert.ToInt32(ReportRow[Column]);
                        int Value = Convert.ToInt32(HandGeneratedRow[Column]);
                        Assert.AreEqual(Compare, Value, Setnb + "/" + Year.ToString() + "/" + Column + " -- hand generated has " + Value.ToString() + ", report has " + Compare.ToString());
                    }
                }
            }
